      ...
      <Container maxWidth="xl">
        <Toolbar disableGutters>
          ...
          <Box sx={{ flexGrow: 1, display: { xs: 'flex', md: 'none' } }}>
            ...
            >
              {navItems.map((item) => (
                <MenuItem key={item.path} onClick={handleCloseNavMenu} component={Link} to={item.path}>
                  <Typography textAlign="center">{item.label}</Typography>
                </MenuItem>
              ))}
            </Menu>
          </Box>
          <BoltIcon sx={{ display: { xs: 'flex', md: 'none' }, mr: 1, justifyContent: 'flex-end', color: 'primary.main' }} />
          <Typography
            variant="h5"
            noWrap
            component="div"
